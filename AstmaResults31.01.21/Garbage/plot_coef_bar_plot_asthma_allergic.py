...
file = os.path.join("..","AstmaResults31.01.21", "astma_allergic", "significant",
                    "significant_homozygots_allergic_astma.csv")
...
for i in range(len(x_pos)):
    ...
    if type == "all":
        bar_plot[i].set_color('darkgreen')
    else:
        bar_plot[i].set_color('lime')
rects = bar_plot
for rect in rects:
    height = rect.get_height()
    ax.text(rect.get_x() + rect.get_width() / 2., - 0.006 + height,
            f"{height : .3f}",
            ha='center', va='bottom')
...
plt.savefig("homozygous_allergic_asthma.jpeg")
plt.show()
